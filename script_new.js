...
// 成就系统
const achievements = [
    { id: 'first_game', name: 'First Try', description: 'Complete your first game', condition: stats => stats.gamesPlayed >= 1, unlocked: false },
    { id: 'score_100', name: 'Beginner Collector', description: 'Score 100 points in a single game', condition: () => score >= 100, unlocked: false },
    { id: 'score_500', name: 'Advanced Collector', description: 'Score 500 points in a single game', condition: () => score >= 500, unlocked: false },
    { id: 'level_5', name: 'Advanced Player', description: 'Reach level 5', condition: () => level >= 5, unlocked: false },
    { id: 'games_10', name: 'Loyal Player', description: 'Play 10 games', condition: stats => stats.gamesPlayed >= 10, unlocked: false }
];
...
function initGame() {
    ...
    pauseOverlay.innerHTML = '<h2>Game Paused</h2><p>Press ESC to continue</p>';
...
function endGame() {
    ...
    gameStats.highestLevel = Math.max(gameStats.highestLevel, level);
    localStorage.setItem('colorCatcherStats', JSON.stringify(gameStats));
    ...
    if (avgScoreElement) avgScoreElement.textContent = Math.round(gameStats.totalScore / gameStats.gamesPlayed);
    if (highestLevelElement) highestLevelElement.textContent = gameStats.highestLevel;
...
function levelUp() {
    ...
    levelUpMessage.textContent = `Level up to ${level}!`;
...
function applyTheme(themeName) {
    ...
    if (gameContainer) {
        gameContainer.style.backgroundColor = theme.container;
    }
    
    const gameAreaElement = document.getElementById('game-area');
    if (gameAreaElement) {
        gameAreaElement.style.backgroundColor = theme.gameArea;
    }
    ...
    localStorage.setItem('colorCatcherTheme', themeName);
}
...
function showAchievements() {
    ...
    if (achievementsContainer && achievementsList) {
        ...
        achievements.forEach(achievement => {
            ...
            achievementItem.className = `achievement-item ${userAchievements[achievement.id] ? 'achievement-unlocked' : 'achievement-locked'}`;
            
            achievementItem.innerHTML = `
                <h3>${achievement.name}</h3>
                <p>${achievement.description}</p>
                <div class="achievement-status">${userAchievements[achievement.id] ? 'Unlocked' : 'Locked'}</div>
            `;
